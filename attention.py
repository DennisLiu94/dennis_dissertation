...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.attention_input_size=self.input_size
        self.other_feature_except_wordemb=self.pos_sen_x[:,self.emb.D:,:]#batchsize*feature_size*time_step
        self.feature_size=self.input_size-self.emb.D
        ##############
        ...
        self.attention_input=self.pos_sen_x.dimshuffle(2,0,1)
        ...
        self.attentionLayer=HiddenLayer(
                ...
                n_in=self.attention_input_size,
                n_out=1,
                )
...
def SEexp(state, channel):
    ...
    attention_file=open('./attention_file','w')
    ...
    temp_dir='.'
    permutation_file=open('permutation_attention','w')
    for epoch_count in xrange(1, n_epoches + 1):
        ...
        permutation_file.write(str(order.tolist()))
      ...
      #  pdb.set_trace()
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                e_pos=0-pos_x[:,0] 
                avg_test_loss,avg_y_pred,avg_y_x,attention_weight=coling.test(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
                test_loss+=avg_test_loss
                if epoch_count % 50 == 0:
                    attention_file.write(str(i)+'\n')
                attention_file.write(str(attention_weight.tolist()))
                if y_pred is None:
                    y_pred=avg_y_pred
                else:
