...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.e1=self.e1_pos_emb.E[self.qz_pos[:,0]]
        self.e2=self.e2_pos_emb.E[self.qz_pos[:,1]]
        ...
        self.pos_sen_x=T.concatenate([self.emb.E[self.sdp],self.e1,self.e2],axis=2)
        ...
        self.input_size=self.emb.D+self.e1_pos_emb.D*2
        ...
        self.lstm_layer_2=BLSTMLayer(
                                rng=rng,
                               input=self.lstm_layer.all_output,
                               input_size=self.lstm_layer.output_size,
                               layers=layers[1:2],
                                activation=model.rectify
                                )
        self.lstm_layer_3=BLSTMLayer(
                                rng=rng,
                               input=self.lstm_layer_2.all_output,
                               input_size=self.lstm_layer_2.output_size,
                               layers=layers[2:],
                                activation=model.rectify
                                )
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        ...
        (lstm_output_1,lstm_output_size_1)=double_max_without_entity(self.lstm_layer.all_output.dimshuffle(1,2,0),self.e_pos,self.qz_senlen,self.lstm_layer.output_size,add_total=False)
        (lstm_output_2,lstm_output_size_2)=double_max_without_entity(self.lstm_layer_2.all_output.dimshuffle(1,2,0),self.e_pos,self.qz_senlen,self.lstm_layer_2.output_size,add_total=False)
        (lstm_output_3,lstm_output_size_3)=double_max_without_entity(self.lstm_layer_3.all_output.dimshuffle(1,2,0),self.e_pos,self.qz_senlen,self.lstm_layer_3.output_size,add_total=False)
        lstm_output=T.concatenate([lstm_output_1,lstm_output_2,lstm_output_3],axis=1)
        lstm_output_size=lstm_output_size_1+lstm_output_size_2+lstm_output_size_3
        ...
        self.debug_f1=theano.function(
                    inputs=[self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.e_pos,self.sen_len,self.is_train],
                    outputs=[
       ...
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.classify=theano.function(
                    inputs=[self.qz_senlen,self.qz_pos,self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.e_pos,self.sen_len,self.is_train],
                    outputs=self.output_layer.y_pred,
                    on_unused_input='ignore'
                    )
        print 'b'
        self.validate=theano.function(
                    inputs=[self.qz_senlen,self.qz_pos,self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.y,self.e_pos,self.sen_len,self.is_train],
...
def SEexp(state, channel):
    ...
    ordered_dataset=qzl.load_data('data/extended_sdp_dataset')
    sdps=ordered_dataset['word_id_sen']
    qz_pos=ordered_dataset['pos']
    train_sdp=sdps[:8000]
    test_sdp=sdps[8000:]
    train_qz_pos=qz_pos[:8000]
    test_qz_pos=qz_pos[8000:]
    qz_senlen=ordered_dataset['sen_len']
    ...
    for epoch_count in xrange(1, n_epoches + 1):
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            qsl=train_qz_senlen[i*batchsize:(i+1)*batchsize]
            qp=train_qz_pos[i*batchsize:(i+1)*batchsize]
            ...
            minibatch_avg_cost,train_y_pred,train_y_x=coling.train(qsl,qp,sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                qp=test_qz_pos[i*size:(i+1)*size]
                qsl=test_qz_senlen[i*size:(i+1)*size]
                e_pos=0-qp[:,0] 
                avg_test_loss,avg_y_pred,avg_y_x=coling.test(qsl,qp,t_sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(sdp_file='./data/full.sdp_idx',loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=1000,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400,200,100],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
