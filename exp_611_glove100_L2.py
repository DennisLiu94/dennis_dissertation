...
class BLSTMLayer(object):
    def __init__(self,rng,input,input_size,layers,activation):
        ...
        self.L2=self.forward_model.L2+self.backward_model.L2
...
class RMM_BLSTM(object):
    ...
    def get_train_fn(self):
        ...
        ##################
        # get cost
        L2=self.lstm_layer.L2+(self.dropout_hiddenLayer.W**2).sum()+(self.dropout_output_layer.W**2).sum()
        self.cost=self.dropout_output_layer.negative_log_likelihood(self.y)+0.001*L2
        ###########
        #  update
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=350,batchsize=200,sen_dropout=0.0,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
