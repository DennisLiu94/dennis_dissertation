...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        #  model building
        #state.word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path:':5}
        self.input_size=self.emb.D*cs+word_fea_emb['ner']+word_fea_emb['pos_label']+word_fea_emb['wnsyn']+word_fea_emb['dep']+word_fea_emb['dep_path']*3
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        ...
        lstm_output=self.lstm_layer.output
        lstm_output_size=self.lstm_layer.output_size
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        emb_params=[self.pos_label_emb.E,self.ner_emb.E,self.wnsyn_emb.E,self.dep_emb.E,self.dep_path_emb.E,self.emb.E]
    ...
    def get_embedding_sen(self,sen_idx,word_emb,pos_idx,pos_max,cs,e1_pos_emb,e2_pos_emb,pos_label_idx,pos_label_emb,ner_idx,ner_emb,wnsyn_idx,wnsyn_emb,dep_idx,dep_emb,dep_path_idx,dep_path_emb):
        ...
        pos_sen_x=T.concatenate((sen_x,word_fea),axis=2).dimshuffle(0,2,1)#dimshuffle(1,0,2)#.dimshuffle(0,2,1)
        return pos_sen_x
...
if __name__ == '__main__':
    ...
    test=True
    if test:
        launch(loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95_mini',n_epoches=3,batchsize=13,
        ...
        launch(loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=200,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
