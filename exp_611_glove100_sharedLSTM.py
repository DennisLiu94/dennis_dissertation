...
class BLSTMLayer(object):
    def __init__(self,rng,input,input_size,layers,activation):
        ...
        self.backward_model=LSTM(
                        input_size=input_size,
                        hidden_size=layers[-1],
                        activation=activation,
                        clip_gradients=self.forward_model.clip_gradients,
                        in_p=[self.forward_model.in_W,self.forward_model.in_b],
                        in_gate_p=[self.forward_model.in_gate_W,self.forward_model.in_gate_b],
                        f_gate_p=[self.forward_model.f_gate_W,self.forward_model.f_gate_b],
                        out_gate_p=[self.forward_model.out_gate_W,self.forward_model.out_gate_b]
                        )
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=350,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[800],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
