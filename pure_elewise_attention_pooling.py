...
def get_attention_weight(attention_output):
    def step(x):
        tmp=T.nnet.softmax(x)
        return tmp
    res,updates=theano.scan(step,sequences=[attention_output],outputs_info=None)
    return res
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        ...
        self.attentionLayer=HiddenLayer(
                ...
                n_out=self.lstm_layer.output_size,
                )
        self.attention_output=self.attentionLayer.output.dimshuffle(2,1,0)
        
        self.attention_weight=get_attention_weight(self.attention_output)
        self.attention_bias=self.attention_weight.max(axis=2)+self.attention_weight.min(axis=2)
        self.attention_bias=self.attention_weight/2.0
        self.attention_weight=self.attention_weight-self.attention_bias
        self.attention_weight=T.maximum(self.attention_weight,0.)*10.0
        (lstm_output,lstm_output_size)=double_max_without_entity(self.lstm_layer.all_output.dimshuffle(1,2,0)*self.attention_weight.dimshuffle(1,0,2),self.e_pos,self.sen_len,self.lstm_layer.output_size,add_total=False)
