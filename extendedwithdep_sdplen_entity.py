...
def entity_fea(input,e_pos,input_size):
    ...
    return (result,input_size*2)
def simple_max(all_output,size):
    res=T.max(all_output,axis=0)
    return res,size
...
class RMM_BLSTM(object):
    ...
    '''
    def __init__(self,deprel_num,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        '''
        RMM_BLSTM
        '''
        ######################
        #  var def
        self.qz_senlen=T.ivector('qz_senlen')
        self.qz_pos=T.itensor3('qz_pos')
        self.deprel=T.imatrix('deprel')
        self.pos_idx=T.itensor3('pos_idx')
        self.sen_idx=T.itensor3('sen_idx')
        self.sdp=T.imatrix('sdp')
        ...
        self.sdplen=T.ivector('sdplen')
        self.sdplen_emb=Embeddings(rng,8,4,'sdplen_emb')
        ...
        self.deprel_emb=Embeddings(rng,deprel_num,15,'deprel_emb')
        '''
        ...
        self.pos_sen_x=T.concatenate([self.emb.E[self.sdp],self.deprel_emb.E[self.deprel],self.e1_pos_emb.E[self.qz_pos[:,:,0]],self.e2_pos_emb.E[self.qz_pos[:,:,1]]],axis=2)
        self.input_size=self.emb.D+self.deprel_emb.D+self.e1_pos_emb.D*2
        self.lex_x,self.lex_x_size=entity_fea(self.pos_sen_x.dimshuffle(0,2,1),self.e_pos,self.input_size)
        self.lstm_input=self.pos_sen_x.dimshuffle(1,0,2)
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        #(lstm_output,lstm_output_size)=simple_max(self.lstm_layer.all_output,self.lstm_layer.output_size)
        (lstm_output,lstm_output_size)=double_max_without_entity(self.lstm_layer.all_output.dimshuffle(1,2,0),self.e_pos,self.qz_senlen,self.lstm_layer.output_size,add_total=False)
        ...
        self.debug_f1=theano.function(
                    inputs=[self.sdplen,self.qz_senlen,self.qz_pos,self.deprel,self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.e_pos,self.sen_len,self.is_train],
                    outputs=[
        ...
        self.sdpleninput=self.sdplen_emb.E[self.sdplen]
        lex_fea_input=T.concatenate([self.lex_x,lstm_lex],axis=1)
        dropout_lex_fea_input=model._dropout_from_layer(rng,lex_fea_input,p=sen_dropout)
        lex_fea_input_size=self.lex_x_size+lstm_lex_size
        self.dropout_hiddenLayer=DropoutHiddenLayer(
                                    rng=rng,
                                    input=T.concatenate([dropout_lstm_output,dropout_lex_fea_input,self.sdpleninput],axis=1),
                                    n_in=lstm_output_size+lex_fea_input_size+self.sdplen_emb.D,
        ...
        self.hiddenLayer=HiddenLayer(
                            rng=rng,
                            input=T.concatenate([lstm_output,lex_fea_input,self.sdpleninput],axis=1),
                            n_in=lstm_output_size+lex_fea_input_size+self.sdplen_emb.D,
                            #input=lstm_output,
                            #n_in=lstm_output_size,
                            n_out=n_hidden,
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        emb_params=[self.deprel_emb.E,self.e1_pos_emb.E,self.e2_pos_emb.E]
        self.params=layer_params#+emb_params
        ####################
        #    debug function
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.validate=theano.function(
                    inputs=[self.sdplen,self.qz_senlen,self.qz_pos,self.deprel,self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.y,self.e_pos,self.sen_len,self.is_train],
...
def SEexp(state, channel):
    ...
    sdps=qzl.load_data(state.sdp)
    sdp=sdps['sen_idx']
    sdp_deprel=sdps['rel_idx']
    sdp_qz_len=sdps['sdp_len']
    qz_senlen=sdps['sen_len']
    qz_pos=sdps['pos']
    train_sdplen=sdp_qz_len[:8000]
    test_sdplen=sdp_qz_len[8000:]
    train_qz_senlen=qz_senlen[:8000]
    test_qz_senlen=qz_senlen[8000:]
    train_qz_pos=qz_pos[:8000]
    test_qz_pos=qz_pos[8000:]
    train_sdp=sdp[:8000]
    test_sdp=sdp[8000:]
    train_deprel=sdp_deprel[:8000]
    test_deprel=sdp_deprel[8000:]
    ...
    #pdb.set_trace()
    emb_lines=qzl.load_data('./data/qz_emb.pkl')
    voca=len(emb_lines)
    if True:
        coling=RMM_BLSTM(
            deprel_num=144,
        ...
        coling.emb=coling.init_emb(emb_lines,voca,coling.emb)
    ...
    for epoch_count in xrange(1, n_epoches + 1):
        ...
        train_sdp=train_sdp[order]
        train_deprel=train_deprel[order]
        train_qz_senlen=train_qz_senlen[order]
        train_qz_pos=train_qz_pos[order]
        train_sdplen=train_sdplen[order]
        if epoch_count%30==0:
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            tr_q_senlen=train_qz_senlen[i*batchsize:(i+1)*batchsize]
            tr_q_pos=train_qz_pos[i*batchsize:(i+1)*batchsize]
            ...
            e_pos=0-tr_q_pos[:,0,:]
            sdp=train_sdp[i*batchsize:(i+1)*batchsize]
            tr_deprel=train_deprel[i*batchsize:(i+1)*batchsize]
            tr_sdplen=train_sdplen[i*batchsize:(i+1)*batchsize]
            #pdb.set_trace()
            ...
            minibatch_avg_cost,train_y_pred,train_y_x=coling.train(tr_sdplen,tr_q_senlen,tr_q_pos,tr_deprel,sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                te_q_senlen=test_qz_senlen[i*size:(i+1)*size]
                te_q_pos=test_qz_pos[i*size:(i+1)*size]
                t_sdp=test_sdp[i*size:(i+1)*size]
                te_deprel=test_deprel[i*size:(i+1)*size]
                te_sdplen=test_sdplen[i*size:(i+1)*size]
                e_pos=0-te_q_pos[:,0]
                avg_test_loss,avg_y_pred,avg_y_x=coling.test(te_sdplen,te_q_senlen,te_q_pos,te_deprel,t_sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
...
def launch(sdp_file,datapath='./data/', dataset='coling_v3_input', dicts='dicts.pkl',idx2emb='idx2emb.pkl',rel_dict='rel_dict.pkl',Nrel=19,batchsize=400,n_epoches=1000,sen_dropout=0.4,lex_dropout=0.1,valid_freq=1,seed=123,savepath='./result',loadmodel=False,normalize=False,layers=[10],suffix='',word_fea_emb=None):
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(sdp_file='./data/extended_deprel_dataset',loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=1000,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
