...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        self.other_model=T.fvector('other_model')
        self.dropout_output_layer=qzm.ClassificationByRanking(
        ...
        self.output_layer=qzm.ClassificationByRanking(
    ...
    def get_train_fn(self):
        ...
        self.cost=self.dropout_output_layer.loss_function(self.y)
        ###########
        #  update
        ...
        train_fn=theano.function(
                inputs=[self.other_model,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.y,self.e_pos,self.sen_len,self.is_train],
                outputs=[self.errors(self.y),self.output_layer.y_pred,self.output_layer.p_y_given_x],
                updates=updates,
                on_unused_input='ignore')
        return train_fn
...
def SEexp(state, channel):
    ...
    for epoch_count in xrange(1, n_epoches + 1):
      ...
      #  pdb.set_trace()
        ...
        train_loss=0
        other_model=np.asarray([0 for _num in range(1000)],dtype=theano.config.floatX)
        for i in range(nbatches):
            ...
            minibatch_avg_cost,train_y_pred,train_y_x=coling.train(other_model,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
