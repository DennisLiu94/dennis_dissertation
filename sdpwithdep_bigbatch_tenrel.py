...
class RMM_BLSTM(object):
    ...
    def __init__(self,deprel_num,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.pos_sen_x=T.concatenate([self.emb.E[self.sdp],self.deprel_emb.E[self.deprel]],axis=2)
        ...
        self.input_size=self.emb.D+self.deprel_emb.D
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        ...
        self.debug_f1=theano.function(
                    inputs=[self.deprel,self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.e_pos,self.sen_len,self.is_train],
                    outputs=[
       ...
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.validate=theano.function(
                    inputs=[self.deprel,self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.y,self.e_pos,self.sen_len,self.is_train],
...
def qz_eval(test_id,test_y,y_pred,idx2rel):
    rels=[]
    for idx in idx2rel.keys():
        rels.append(idx2rel[idx])
    f1_list=[]
    for idx in idx2rel.keys():
        tmp=''
        tmp+=str(idx2rel[idx])+' p,r,f1'
        total_p=0
        total_r=0
        hit=0
        for i,_id in enumerate(test_id):
            if test_y[i]==idx:
                total_r+=1
            if y_pred[i]==idx:
                total_p+=1
            if test_y[i]==y_pred[i] and y_pred[i]==idx:
                hit+=1

        p=hit/float(total_p) if total_p != 0 else 0
        r=hit/float(total_r) if total_r!=0 else 0
        f1=2*p*r/(p+r) if (p+r)!=0 else 0
        tmp+=':%f,%f,%f\n'%(p,r,f1)
        #print tmp
        f1_list.append(f1)
    #print 'macro f1:%f\n'%(sum(f1_list)/float(len(f1_list)))
    return sum(f1_list)/float(len(f1_list))
...
def SEexp(state, channel):
    ...
    tenrel2idx,idx2tenrel=dicts['tenrel_dict']
    tworel2idx,idx2tworel=dicts['tworel_dict']
    ...
    train_tenrel=data_set['train']['tenrel']
    train_tworel=data_set['train']['tworel']
    #dependency fea
    ...
    test_tenrel=data_set['test']['tenrel']
    test_tworel=data_set['test']['tworel']
    #dependency fea
    ...
    for epoch_count in xrange(1, n_epoches + 1):
        ...
        train_tenrel=train_tenrel[order]
        train_tworel=train_tworel[order]
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            ...
            y=train_tenrel[i*batchsize:(i+1)*batchsize]
            ...
            minibatch_avg_cost,train_y_pred,train_y_x=coling.train(tr_deprel,sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                avg_test_loss,avg_y_pred,avg_y_x=coling.test(te_deprel,t_sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
            ...
            test_loss/=batchs
            epoch_evaluate(y_pred,test_tenrel,idx2tenrel)
            
            #pdb.set_trace()
            f1=qz_eval(test_id,test_tenrel,y_pred,idx2rel)
            print '\nepoch %d\ttest loss:%f\ttrain_loss:%f\tf1:%f\n'%(epoch_count,test_loss*100,train_loss*100,f1)
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(sdp_file='./data/sdp_with_deprel_dataset',loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=1000,batchsize=1000,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
