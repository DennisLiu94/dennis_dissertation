...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        '''
        self.pos_sen_x=T.concatenate([self.emb.E[self.sen_idx[:,:,1]],self.e1_pos_emb.E[self.pos_idx[:,:,0]],self.e2_pos_emb.E[self.pos_idx[:,:,1]],self.ner_emb.E[self.ner_idx],self.dep_emb.E[self.dep_idx],self.wnsyn_emb.E[self.wnsyn_idx],self.pos_label_emb.E[self.pos_label_idx]],axis=2).dimshuffle(0,2,1)
        ...
        self.input_size=self.emb.D+self.e1_pos_emb.D+self.e2_pos_emb.D+self.ner_emb.D+self.dep_emb.D+self.wnsyn_emb.D+self.pos_label_emb.D
        ##############
        #remove slip window
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        emb_params=[self.e1_pos_emb.E,self.e2_pos_emb.E,self.pos_label_emb.E,self.ner_emb.E,self.wnsyn_emb.E,self.dep_emb.E,self.emb.E]
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=700,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
