...
def double_max_without_entity(input,e_pos,sen_len,input_size,add_total=True):
    ...
    def step(h,e,s_len):
        ...
        max1=T.switch(T.gt(max1_area.shape[1],2),max1_area.sum(axis=1),pad)
        ...
        max2=T.switch(T.gt(max2_area.shape[1],2),max2_area.sum(axis=1),pad)
       
        total_max=T.concatenate([max1_area,max2_area],axis=1).sum(axis=1)
        if add_total:
...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.attention_input=self.lstm_input
        attention_input_size=lstm_input_size
        self.lstm_input=self.lstm_input[:,:,:self.emb.D]
        lstm_input_size=self.emb.D
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        ...
        self.attentionLayer=HiddenLayer(
                rng=rng,
                input=self.attention_input,
                n_in=attention_input_size,
                n_out=1,
                )
        self.attention_output=self.attentionLayer.output[:,:,0].dimshuffle(1,0)
        self.attention_weight=T.nnet.softmax(self.attention_output)
        self.attention_bias=self.attention_weight.max(axis=1)+self.attention_weight.min(axis=1)
        self.attention_bias=self.attention_bias/2.0
        self.attention_weight=self.attention_weight-self.attention_bias.dimshuffle(0,'x')
        self.attention_weight=T.maximum(self.attention_weight,0.)*10
        
        (lstm_output,lstm_output_size)=double_max_without_entity(self.lstm_layer.all_output.dimshuffle(1,2,0)*self.attention_weight.dimshuffle(0,'x',1),self.e_pos,self.sen_len,self.lstm_layer.output_size,add_total=False)
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        layer_params=self.lstm_layer.params+self.dropout_hiddenLayer.params+self.dropout_output_layer.params+self.attentionLayer.params
       ...
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.validate=theano.function(
                    ...
                    outputs=[self.errors(self.y),self.output_layer.y_pred,self.output_layer.p_y_given_x,self.attention_weight],
                    on_unused_input='ignore'
                    )
      ...
      #  pdb.set_trace()
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                e_pos=0-pos_x[:,0] 
                avg_test_loss,avg_y_pred,avg_y_x,att_weight=coling.test(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(loadmodel=True,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=350,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
