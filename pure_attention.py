    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.validate=theano.function(
                    ...
                    outputs=[self.attention_weight,self.errors(self.y),self.output_layer.y_pred,self.output_layer.p_y_given_x],
                    on_unused_input='ignore'
                    )
...
def SEexp(state, channel):
    ...
    for epoch_count in xrange(1, n_epoches + 1):
        ...
        if epoch_count==330:
            attention_file.write(str(order.tolist()))
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            ...
            attention_weight,minibatch_avg_cost,train_y_pred,train_y_x=coling.train(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                e_pos=0-pos_x[:,0] 
                attention_weight,avg_test_loss,avg_y_pred,avg_y_x=coling.test(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
