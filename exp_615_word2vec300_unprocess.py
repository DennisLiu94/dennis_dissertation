...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.pos_sen_x=self.emb.E[self.sen_idx].reshape((self.sen_idx.shape[0],self.sen_idx.shape[1],self.emb.D*cs)).dimshuffle(0,2,1)
        self.pos_sen_x=self.pos_sen_x[:,self.emb.D:self.emb.D*2,:]
...
if __name__ == '__main__':
    ...
    test=False
    if test:
        launch(loadmodel=False,idx2emb='word2vec_300_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95_mini',n_epoches=3,batchsize=13,
        ...
        launch(loadmodel=False,idx2emb='word2vec_300_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=300,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
