...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.lstm_layer_origin=BLSTMLayer(
                                rng=rng,
                               input=self.lstm_input,
                               input_size=lstm_input_size,
                                layers=layers,
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        (lstm_output_origin,lstm_output_size_origin)=double_max_without_entity(self.lstm_layer_origin.all_output.dimshuffle(1,2,0),self.e_pos,self.sen_len,self.lstm_layer_origin.output_size,add_total=False)
        ...
        dropout_lstm_output_origin=model._dropout_from_layer(rng,lstm_output_origin,p=sen_dropout)
        ...
        self.dropout_hiddenLayer_origin=DropoutHiddenLayer(
                                    rng=rng,
                                    input=dropout_lstm_output_origin,
                                    n_in=lstm_output_size_origin,
                                    #input=dropout_lstm_output,
                                    #n_in=lstm_output_size,
                                    n_out=n_hidden,
                                    dropout_rate=sen_dropout,
        ...
        self.hiddenLayer_origin=HiddenLayer(
                            rng=rng,
                            input=lstm_output_origin,
                            n_in=lstm_output_size_origin,
                            #input=lstm_output,
                            #n_in=lstm_output_size,
                            n_out=n_hidden,
                            W=self.dropout_hiddenLayer_origin.W*(1-sen_dropout),
                            b=self.dropout_hiddenLayer_origin.b,
                            activation=model.rectify
                            )
        ###############################
       # build other layers
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        self.dropout_output_layer=LogisticRegression(
                ...
                input=T.concatenate([self.dropout_hiddenLayer.output,self.dropout_hiddenLayer_origin.output],axis=1),
                n_in=n_hidden*2,
               # W=logic_W,
                n_out=label_num
            )
        ...
        self.output_layer=LogisticRegression(
                ...
                #n_in=n_hidden+lex_fea_input_size,
                input=T.concatenate([self.hiddenLayer.output,self.hiddenLayer_origin.output],axis=1),
                n_in=n_hidden*2,
        ...
        layer_params=self.lstm_layer.params+self.dropout_hiddenLayer.params+self.dropout_output_layer.params+self.lstm_layer_origin.params+self.dropout_hiddenLayer_origin.params
       ...
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.t_pos_value_list=T.concatenate(self.pos_value_list) 
        ...
        self.validate=theano.function(
                    ...
                    outputs=[self.t_pos_value_list,self.errors(self.y),self.output_layer.y_pred,self.output_layer.p_y_given_x],
                    on_unused_input='ignore'
                    )
...
def SEexp(state, channel):
    ...
    temp_dir=__file__+'_res/'
    if not os.path.exists(temp_dir):
        os.makedirs(temp_dir)
    attention_pos_list=[]
    for epoch_count in xrange(1):
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            ...
            attention_pos,minibatch_avg_cost,train_y_pred,train_y_x=coling.train(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
            train_loss+=minibatch_avg_cost
            attention_pos_list.append(attention_pos)
            if state.normalize:
                coling.normalize()
        #pdb.set_trace()
        train_loss/=nbatches
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                e_pos=0-pos_x[:,0] 
                attention_pos,avg_test_loss,avg_y_pred,avg_y_x=coling.test(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
    ...
    print '-'*5+'test f1:'+'-'*10
    print test_f1_list
    qzl.dump_data(attention_pos_list,'qz_res/attention_pos.pkl')
    save_data((train_loss_list,test_loss_list,test_f1_list),state.result_list_file)
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(loadmodel=True,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=700,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[400],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
