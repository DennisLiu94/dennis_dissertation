...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        (lstm_output_max,lstm_output_size_max)=double_max_without_entity(self.lstm_layer.all_output.dimshuffle(1,2,0),self.e_pos,self.sen_len,self.lstm_layer.output_size,add_total=False)
        (lstm_lex,lstm_lex_size)=entity_fea(self.lstm_layer.all_output.dimshuffle(1,2,0),self.e_pos,self.lstm_layer.output_size)
        self.lstm_output_list=[lstm_lex]
        ...
        for attetion_count in range(3):
        
            self.attention_value_list.append(self.attention_layer.get_output(input=T.concatenate([self.lstm_layer.all_output,T.concatenate([self.lstm_output_list[-1].dimshuffle('x',0,1) for i in range(95)],axis=0)],axis=2)))
        ...
        lstm_output=T.concatenate(self.lstm_output_list+[lstm_output_max],axis=1)
        
        
        
        lstm_output_size=self.lstm_layer.output_size*len(self.lstm_output_list)*2+lstm_output_size_max
...
if __name__ == '__main__':
    ...
    if test:
        ...
        launch(loadmodel=False,idx2emb='glove_100_idx2emb.pkl',datapath='./data',dataset='blstm_complete_input_len95',n_epoches=350,batchsize=200,sen_dropout=0.5,lex_dropout=0.0,layers=[100],suffix='',word_fea_emb={'pos':5,'pos_label':20,'ner':20,'wnsyn':20,'dep':20,'dep_path':10, 'com_dep_path':20})
