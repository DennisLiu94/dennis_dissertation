...
def last_step(forward_input,backward_input,sen_len,input_size):
    #make sure input.shape=(batch,dim,step)
    #e.shape=(batch,[e1,e2])
    #sen_len.shape=(batch,)
    def step(f_in,b_in,s_len):
        f_fea=f_in[:,s_len]
        b_fea=b_in[:,0]
        return T.concatenate([f_fea,b_fea])
    
    result,update=theano.scan(
                            step,
                            sequences=[forward_input,backward_input,sen_len],
                            outputs_info=None
                           )
...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        (lstm_output,lstm_output_size)=last_step(self.lstm_layer.forward_output.dimshuffle(1,2,0),self.lstm_layer.forward_output.dimshuffle(1,2,0),self.sen_len,self.lstm_layer.output_size)
