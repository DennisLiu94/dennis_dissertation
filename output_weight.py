...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        ...
        self.pos_list=[t.argmax(axis=1).dimshuffle(0,'x') for t in self.pos_value_list]
        self.attention_output=T.concatenate(self.pos_list,axis=1)
        
        self.output_attention=theano.function(
...
def SEexp(state, channel):
    ...
    for epoch_count in range(1):
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            ...
            output_weight=coling.output_weight(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
            #train_loss+=minibatch_avg_cost
            output_list.append(output_weight)
            if state.normalize:
                coling.normalize()
        #pdb.set_trace()
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                e_pos=0-pos_x[:,0] 
                output_weight=coling.output_weight(sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
    ...
    qzl.dump_data(output_list,'data/attention_output.pkl')
