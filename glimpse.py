...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
       ...
       # self.debug_check_lstm_output=theano.function([self.sen_idx,self.pos_idx],[self.lstm_layer.all_output])
        self.attention_chain_shallow=qz_model.attention_chain(self.lstm_layer.all_output,self.lstm_layer.output_size,self.e_pos,self.sen_len,rng)
        lstm_output=self.attention_chain_shallow.output
        lstm_output_size=self.attention_chain_shallow.output_size
        '''

        self.g_n=qz_model.glimpse_network(
                    sen_input=self.pos_sen_x.dimshuffle(0,2,1),
                    sen_size=self.input_size,
                    rng=rng,
                    hidden_size=100
                )
        lstm_output,lstm_output_size=self.g_n.get_output()
        ...
        '''
        lex_fea_input=T.concatenate([self.lex_x],axis=1)
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        layer_params=self.g_n.params+self.dropout_hiddenLayer.params+self.dropout_output_layer.params
