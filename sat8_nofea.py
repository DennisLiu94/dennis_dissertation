...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        '''
        input_list=[self.sen_idx[:,:,1],self.pos_idx[:,:,0],self.pos_idx[:,:,1]]
        input_emb=[self.emb,self.e1_pos_emb,self.e2_pos_emb]
        input_emb_list=map(lambda x:x[1].E[x[0]],zip(input_list,input_emb))
        self.pos_sen_x=T.concatenate(input_emb_list,axis=2).dimshuffle(0,2,1)
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        emb_params=[self.emb.E,self.e1_pos_emb.E,self.e2_pos_emb.E]
