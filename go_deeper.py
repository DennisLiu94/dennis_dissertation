...
class RMM_BLSTM(object):
    ...
    def __init__(self,rng,word_num,emb_dim,sen_len,cs,pos_max,pos_emb_len,lex_len,ex_fea_len,conv_output_col,n_hidden,label_num,sen_dropout,lex_dropout,layers,max_word_fea_size,word_fea_emb):
        ...
        self.pos_sen_x=self.emb.E[self.sdp]
       ...
       # logic_W=theano.shared(value=logic_W_values,name='W',borrow=True)
        ...
        layer_params=self.lstm_layer.params+self.lstm_layer_2.params+self.lstm_layer_3.params+self.dropout_hiddenLayer.params+self.dropout_output_layer.params
       ...
       # self.debug_f=theano.function([self.sen_idx,self.pos_idx],[lstm_output],on_unused_input='ignore')
        ...
        self.validate=theano.function(
                    inputs=[self.sdp,self.sen_idx,self.pos_label_idx,self.ner_idx,self.wnsyn_idx,self.dep_idx,self.dep_path_idx,self.com_dep_path_idx,self.pos_idx,self.y,self.e_pos,self.sen_len,self.is_train],
...
def SEexp(state, channel):
    ...
    sdps=np.asarray(sdps,dtype='int32')
    train_sdp=sdps[:8000]
    test_sdp=sdps[8000:]
    ...
    for epoch_count in xrange(1, n_epoches + 1):
      ...
      #  pdb.set_trace()
        ...
        for i in range(nbatches):
            ...
            minibatch_avg_cost,train_y_pred,train_y_x=coling.train(sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](1))
        ...
        if (epoch_count % state.test_freq)==0:
            ...
            for i in range(batchs):
                ...
                avg_test_loss,avg_y_pred,avg_y_x=coling.test(t_sdp,sen_x,pos_label_x,ner_x,wnsyn_x,dep_x,dep_path_x,com_dep_path_x,pos_x,y,e_pos,sen_len,np.cast['int32'](0))
